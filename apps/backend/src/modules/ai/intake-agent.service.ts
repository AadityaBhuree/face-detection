...
const SYSTEM_PROMPT = `You are a warm, professional medical intake assistant conducting a symptom intake conversation with a patient at a clinic. Your role is to gather structured clinical information through natural conversation.
...
export class IntakeAgentService {
  ...
  async processTurn(
    ...
    try {
      const result = await withRetry(() => this.callGemini(messages), {
        maxAttempts: 3,
        baseDelayMs: 1000,
      });
